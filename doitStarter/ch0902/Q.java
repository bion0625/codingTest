...
public class Q {
    static class BinTree<K, V> {
        ...
        // node를 루트로 하는 서브트리의 노드를 키값의 내림차순으로 출력
        ...
        public K getMinKey() {
            ...
            Node<K, V> p = root;
            while (true) {
                if (p.left == null) return p.getKey();
                else p = p.left;
            }
        }
        ...
        public V getDataWithMinKey() {
            ...
            Node<K, V> p = root;
            while (true) {
                if (p.left == null) return p.getValue();
                else p = p.left;
            }
        }
        ...
        public K getMaxKey() {
            ...
            Node<K, V> p = root;
            while (true) {
                if (p.right == null) return p.getKey();
                p = p.right;
            }
        }
        ...
        public V getDataWithMaxKey() {
            ...
            Node<K, V> p = root;
            while (true) {
                if (p.right == null) return p.getValue();
                p = p.right;
            }
        }
    }
    ...
    static void q2() {
        BinTree<Integer, String> binTree = new BinTree<>();
        binTree.add(4, "data 4");
        binTree.add(7, "data 7");
        binTree.add(10, "data 10");
        binTree.add(5, "data 5");
        binTree.add(3, "data 3");
        binTree.add(2, "data 2");

        System.out.println("minKey: " + binTree.getMinKey());
        System.out.println("minKeyData: " + binTree.getDataWithMinKey());

        System.out.println("maxKey: " + binTree.getMaxKey());
        System.out.println("maxKeyData: " + binTree.getDataWithMaxKey());
    }

    static void q3() {
        BinTree<Integer, String> binTree = new BinTree<>((b1, b2) -> b1 - b2);
        binTree.add(4, "data 4");
        binTree.add(7, "data 7");
        binTree.add(10, "data 10");
        binTree.add(5, "data 5");
